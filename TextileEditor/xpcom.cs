...
namespace Gecko
{
    ...
    public static class Xpcom
    {
        ...
        private static extern int NS_InitXPCOM2(
        ...
        /// <returns></returns>
        [DllImport("xul", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        private static extern int NS_NewLocalFile(
            [MarshalAs(UnmanagedType.CustomMarshaler, MarshalTypeRef = typeof (Gecko.CustomMarshalers.AStringMarshaler))
            ] nsAString path, bool followLinks, [MarshalAs(UnmanagedType.IUnknown)] out object result);
        ...
        /// <returns></returns>
        [DllImport("xul", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        private static extern int NS_GetComponentManager(
            [MarshalAs(UnmanagedType.Interface)] out nsIComponentManager componentManager);
        ...
        {
            get { return Thread.CurrentThread.ManagedThreadId != _XpcomThreadId; }
        }
                ...
                if (_ProfileDirectory == null)
                {
                    string directory =
                        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                            Path.Combine("Geckofx", "DefaultProfile"));
        ...
        internal delegate IntPtr GetJSObjectFromHolderDelegate(
            [MarshalAs(UnmanagedType.Interface)] nsIXPConnectJSObjectHolder @this);
        ...
        public static object QueryInterface(object obj, Guid iid)
        {
            ...
            if (ppv == IntPtr.Zero)
            {
                ...
                if (pInterfaceRequestor != IntPtr.Zero)
                {
                    ...
                    QI_nsIInterfaceRequestor req =
                        (QI_nsIInterfaceRequestor) Xpcom.GetObjectForIUnknown(pInterfaceRequestor);
